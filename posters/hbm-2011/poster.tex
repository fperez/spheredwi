...
\hspace{1cm}To visualize how well a point set covers the sphere we assign to
each point a Gaussian density $\exp\left(-s^2/2\sigma^2\right)$, where $s$ is
the arc length measured from the point and $\sigma^2=2/n\log\left(2\right)$
with $n$ the number of points. Fig.~1 shows a coverage map of a symmetrized
version of a 64 point grid regularly used for diffusion tensor imaging at the
UC Berkeley Siemens 3T Trio scanner. The 64 sampling directions lie in a
hemisphere, so another 64 points were added to the grid by reflecting the
original ones through the origin. The displayed version thus has 128
points. Fig.~2 shows a coverage map for a quadrature with a total of 72 nodes
(originally distributed over the whole sphere) taken from
Ref.\,\cite{AHR-BEY-2009}.
...
\begin{center}
  ...
  \myfig[0]{coverage_64_full_range.eps}{0.85} \mycaption{Coverage map for 64
    measurement directions (full set of 128 symmetrized points shown).}
\end{center}
%
\begin{center}
  % first argument: eps-file
  ...
  \mycaption{Coverage map for quadrature with a total of 72 nodes from Ref.\,\cite{AHR-BEY-2009}.}
\end{center}
%
...
\hspace{1cm}The quadratures from Ref.\,\cite{AHR-BEY-2009} will now be used to develop a localized representation of functions on the sphere, ideally suited for reconstruction of HARDI signals.
...
\end{equation}
%
with $\bomega_{j}$ a quadrature point and $w_{j}$ the associated weight. If $f\in\HH_{N}$, then Eq.\,(\ref{eq:f-semi-disc}) provides
...
and, therefore, we may think of Eq.\,(\ref{eq:f-semi-disc}) as an analogue
of Lagrange interpolation on the sphere. Fig.~3 shows a plot of one of the functions $K\left(\bomega\cdot\bomega_{j}\right)$ with the point $\bomega_{j}$ taken as the north pole, $\mathbf{e}_z$.
\begin{center}
  % first argument: eps-file
  % second argument: stretching-factor relative to Column-width (<1)
  % optional argument: rotation angle (0-360), default=0
